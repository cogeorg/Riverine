<script>
    ...
    displayStackedOption = false;

    if (_xaxis.id.toLowerCase() != "year"){
        displayStackedOption = true;
    }

    _min = Number(d3.min(org_modelData, function(d) { return d.value; }));
    _max = Number(d3.max(org_modelData, function(d) { return d.value; }));

    // get_param function gets "_param_value" configuration from conf_obj
    // Set _param_value = "xaxis" or "yaxis" or "ID"
