...
class ScatterplotVisualizer(ModelVisualizer):
    ...
    def read_model_data(self):
        ...
        for row in range(len(file_data)):
            ...
            for col in column_index:
                ...
                tmp.update({self._parameters[col]['@value']:value})
    ...
    def render_plot(self):
        self.js = render_template('dynamic_js/scatterplot.js', conf_obj=self.config_obj, data_id = self.data_id,
                        headers=self.headers, modelData=self.modelData, param_keys = self.param_keys)
        self.plot = render_template('ModelVisualizer/scatterplot.html', js = self.js,
